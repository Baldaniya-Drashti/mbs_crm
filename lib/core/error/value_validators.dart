...
Either<ValueFailure<List<T>>, List<T>> validateMaxGuildLength<T>(
  ...
  if (input.length <= maxLength) {
    ...
    return left(ValueFailure.tooManyGuilds(failedValue: input, max: maxLength));
  }
}
...
Either<ValueFailure<File>, File> validateMaxFileSize(File input, int maxSize) {
  if (input.lengthSync() <= maxSize) {
    return right(input);
  } else {
    return left(ValueFailure.exceedingSize(failedValue: input, max: maxSize));
  }
}

Either<ValueFailure<String>, String> validateHexColor(String input) {
